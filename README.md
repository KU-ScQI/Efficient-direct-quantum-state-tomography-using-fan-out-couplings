...
`DQST_full_reconstruction_data.csv` and `DQST_full_reconstruction_data_QREM.csv` contain the raw and QREM-corrected experimental measurement data, respectively, obtained from IBM Quantum (backend: IBM_aachen) for three 4-qubit states: the GHZ state, the computational basis state |0000⟩, and the |++++⟩ state.
...
`GHZ_fidelity_estimation_data_raw.csv` and `GHZ_fidelity_estimation_data_QREM.csv` contain the raw and QREM-corrected experimental measurement data, respectively, obtained from IBM Quantum (backend: IBM_aachen) for system sizes \(n = 4, 5, 6, 7, 8, 9, 10, 15,\) and \(20\), where \(n\) denotes the number of system qubits.
